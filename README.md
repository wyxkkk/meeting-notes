...
把 OBS 录音在本机转成文字，实验性地用 Jev 去除纯闲聊，再将材料交给 Codex 整理中文纪要和待办。

项目用于共同研究方案，目前是命令行原型。文件处理、本机转写和 Jev 调用已经跑通；筛选策略仍在实验，尚未证明节省总 token 或费用。最终纪要需要手动交给 Codex，尚无一键界面或说话人身份识别。

## 当前研究方向

父块提供背景，子片段单独判断。当前实验固定父块 60 秒、前后各 15 秒上下文，只问“目标子片段是否明确属于可去除的纯闲聊”，概率达到 0.90 才列为候选删除。
...
| `selected.md` / `selected.json` | 按阈值保留的原文 |
...
| `threshold-views.json` | 同一批概率在其他阈值下的统计，不额外调用模型 |
